...
\begin{tabular}{lrrr}
\toprule
\textbf{Memory setting} & \textbf{Mean (ms)} & \textbf{P50 (ms)} & \textbf{P95 (ms)} \\
\midrule
64 MiB   & 74.0        & 72.4        & 87.1        \\
256 MiB  & 303.2       & 303.4       & 308.5       \\
1024 MiB & 1{,}234.9   & 1{,}219.8   & 1{,}391.5   \\
8192 MiB & 10{,}534.3  & 10{,}269.6  & 12{,}063.0  \\
\bottomrule
\end{tabular}
\end{table}

\add{Table~\ref{tab:argon2_defender_cost} reports single-invocation latency ($k=1$). When the user applies KDF chaining at depth $k$ (Equation~\ref{eq:kdf_chaining}), the total key-derivation time scales linearly as $T_\text{CPU}(k) = k \cdot T_\text{CPU}^{(1)}$. Table~\ref{tab:kchain_empirical} reports chain latency for powers-of-two depths at the $m=1{,}024$\,MiB reference configuration. The $k=1$ row is directly measured ($n=50$ reps); all other rows are derived via $T(k)=k\cdot T(1)$, a relationship confirmed empirically to within~2\% across $k\in\{3,5,10,30,60\}$.}

\begin{table}[H]
\caption{\add{KDF chain latency by depth ($m=1{,}024$\,MiB, $t=1$, $p=1$). $k=1$: measured ($n=50$); $k\geq2$: derived from $T(k)=k\cdot T(1)$, confirmed to within~2\%.}}
\label{tab:kchain_empirical}
\centering
\begin{tabular}{rr}
\toprule
\textbf{Depth $k$} & \textbf{Mean (s)} \\
\midrule
 1 &  1.235$^*$ \\
 2 &  2.470 \\
 4 &  4.939 \\
...
64 & 79.031 \\
\bottomrule
\multicolumn{2}{l}{\footnotesize $^*$Measured; $\sigma=0.047$\,s. All other rows derived.}
\end{tabular}
\end{table}

\add{From the attacker's perspective, each of the $k$ invocations carries the same per-call memory cost and the serial data dependency (Equation~\ref{eq:kdf_chaining}) prevents evaluating multiple rounds in parallel for a single candidate. Attacker throughput therefore scales as $R_\text{GPU}^{(k)} = R_\text{GPU}^{(1)} / k$, making the work factor grow proportionally with $k$ at no additional memorization cost.}
...
\add{KDF chaining (Equation~\ref{eq:kdf_chaining}) provides a user-controlled work factor multiplier that scales both sides of the security equation linearly. Because each invocation reuses the full $m$-byte memory footprint and the serial data dependency prevents per-candidate parallelism, attacker throughput drops to $R_\text{GPU}^{(k)} = R_\text{GPU}^{(1)} / k$ while defender latency grows to $T_\text{CPU}(k) = k \cdot T_\text{CPU}^{(1)}$ (Table~\ref{tab:kchain_empirical}). Table~\ref{tab:k_chaining_effect} quantifies this trade-off at the $m=1{,}024$\,MiB reference configuration.}

\begin{table}[H]
\caption{\add{Effect of KDF chaining depth $k$ on attacker time-to-compromise and defender wait time at $m=1{,}024$\,MiB ($t=1$, $p=1$, $R^{(1)}\approx53.7$\,H/s). Depths are powers of two; all values derived from measured $k=1$ baseline. Cell shading indicates security zone.}}
...
\midrule
 1 &  1.2\,s & \cellcolor{insecure}5.4\,h    & \cellcolor{humanscale}$3.3\times10^{4}$\,yr \\
 2 &  2.4\,s & \cellcolor{insecure}10.8\,h   & \cellcolor{humanscale}$6.6\times10^{4}$\,yr \\
 4 &  4.9\,s & \cellcolor{insecure}21.6\,h   & \cellcolor{humanscale}$1.3\times10^{5}$\,yr \\
 8 &  9.8\,s & \cellcolor{insecure}1.8\,days  & \cellcolor{humanscale}$2.6\times10^{5}$\,yr \\
16 & 19.5\,s & \cellcolor{insecure}3.6\,days  & \cellcolor{humanscale}$5.3\times10^{5}$\,yr \\
32 & 39.0\,s & \cellcolor{insecure}7.2\,days  & \cellcolor{humanscale}$1.1\times10^{6}$\,yr \\
64 & 78.1\,s ($\approx$1.3\,min) & \cellcolor{insecure}14.4\,days & \cellcolor{humanscale}$2.1\times10^{6}$\,yr \\
\bottomrule
\end{tabular}
\end{table}

\add{Two conclusions follow directly. First, for low-entropy linguistic secrets ($\approx$20 bits), chaining provides no practical rescue: even at $k=64$ the expected time-to-compromise remains below $10^{10}$\,s (Insecure zone), because the entropy deficit is too large for a linear multiplier to overcome. Second, for spatial secrets the picture is reversed: the baseline $k=1$ value of $\approx\!33{,}400$ years is already firmly in the Human-Scale Secure zone, and chaining amplifies this proportionally at negligible memorization overhead. In the combined multi-point setting (Section~\ref{sec:defense_in_depth}), $k$ acts as a fine-grained dial that shifts the entire work-factor curve upward by $k$-fold without requiring additional spatial points or increased memory. Using powers of two for $k$ is consistent with standard cryptographic cost-parameter conventions, where each increment doubles the work factor.}
