...
\hreflink{} % DOI to be filled once journal assigns it
%\doinum{}
...
\address{%
$^{1}$ \quad Faculty of Mechanical Engineering, University of Ljubljana \\
...
\remove{The contributions of this work are threefold:}\add{The main contributions of this work are:}
\begin{itemize}
...
\add{Human memory is notoriously unreliable when it comes to memorizing abstract information such as complex alphanumeric strings or passphrases~\cite{Pals2018, Akaygun2014}. However, decades of cognitive psychology research have shown that humans exhibit a disproportionately strong ability to encode, recall, and retain spatial information~\cite{montello1998new, McNamara2003}. This observation underpins the ``method of loci,'' an ancient memory technique that leverages imagined spatial environments to recall information by placing memories at physical locations within a mental map~\cite{Yates2013}.}
...
\add{More recently, spatial interfaces have been proposed in password systems and digital authentication. Early frameworks such as \textit{PassMap}~\cite{Sun2012} and \textit{GeoPass}~\cite{Thorpe2013} demonstrated that map-based authentication secrets are more durable than abstract textual passwords. In long-term usability studies, users retained geographic authentication secrets for durations of 3--5 weeks with significantly fewer errors compared to alphanumeric passphrases~\cite{Thorpe2013}. Furthermore, \textit{GeoPassNotes}~\cite{MacRae2016} suggests that anchoring secrets to personally meaningful spatial landmarks enhances recall through the activation of episodic memory cues.}

\add{Despite these advantages, prior work in map-based authentication has identified severe entropy collapse caused by predictable human selection. Users consistently exhibit strong semantic bias toward residential locations (home/work), major urban centers, and famous global landmarks~\cite{Sun2012, Thorpe2013, AlAmeen2014}. A comprehensive usability and security evaluation by Al-Ameen and Wright~\cite{AlAmeen2014} further quantified the degree to which user selection distributions concentrate around personally and culturally salient locations, confirming that an adversary with even limited contextual knowledge of the user can substantially reduce the effective search space below the nominal geographic entropy. These selection patterns reduce the effective search space in ways that purely geographic dictionaries do not capture. While these studies establish the memorability of spatial secrets, they have not yet been integrated into a cryptographic key derivation framework that explicitly mitigates selection bias through memory-hard hardening and hardware-aware offline threat modeling.}

\add{These findings suggest that spatial memory may provide a superior foundation for cryptographic key storage and retrieval mechanisms. Unlike traditional brainwallets that rely on memorized strings, a system that anchors a seed derivation process to a spatial coordinate or map location naturally exploits this cognitive strength. By combining the robustness of spatial recall with computational functions that bind access to the correct location and effort, it is possible to achieve both higher security and improved usability.}
...
\subsection{\remove{Encryption Techniques for Burying Secrets} \add{Encryption Techniques for Hardening Secrets}}
...
\add{Secure storage of mnemonic seeds in the \textit{GeoVault} protocol relies on cryptographic techniques that impose significant computational barriers to unauthorized recovery while ensuring practical access for legitimate users. While mechanisms such as verifiable delay functions (VDFs)~\cite{Boneh2018, Wesolowski2019}, time-lock puzzles~\cite{Mahmoody2011}, and proofs of sequential work (PoSW)~\cite{Cohen2018} provide strong temporal and sequential protection, \textit{GeoVault} primarily leverages memory-hard key derivation functions (KDFs) to enforce hardware-bound asymmetry between defenders and offline adversaries.}

\add{Memory-hard KDFs, such as Argon2, transform inputs like spatial coordinates into cryptographic keys through processes that require substantial memory resources, thereby resisting optimization by hardware accelerators such as ASICs or GPUs~\cite{7467361, Wetzels2016}. Unlike compute-bound hashes, Argon2d binds attacker performance to memory bandwidth and capacity. This ensures that even high-end GPU clusters face a non-linear reduction in throughput once the memory requirements of the KDF approach the physical VRAM limits of the device. Although emerging compute-capable memory technologies may eventually reduce access costs for older functions like scrypt~\cite{Choe2019}, Argon2id remains the current standard for mitigating both side-channel and time-memory trade-off attacks in human-centered key management.}

\add{Collectively, these techniques allow \textit{GeoVault} to convert modest user-side latency into a disproportionate increase in attacker work factors. Future work could explore hybrid approaches that combine memory-hardness with PoSW to further enhance resistance against parallelized map-scanning attacks in spatially anchored mnemonic storage.}
...
\add{Equation~\ref{eq:spatial_dict_entropy} models the attacker's \emph{geographic} prior---which dictionary stratum is targeted---but treats selection \emph{within} the targeted stratum as uniform. Empirical studies of geographic authentication systems demonstrate that this uniformity assumption overstates effective entropy: users disproportionately choose home addresses, workplaces, frequently visited venues, and culturally salient landmarks, concentrating realistic choices over a sub-fraction of the dictionary's nominal coverage area~\cite{Sun2012,Thorpe2013,AlAmeen2014}. To incorporate this within-dictionary behavioural bias, we introduce a selection fraction $f_s \in (0,1]$, representing the proportion of cells in $\mathcal{D}$ that attract non-negligible probability mass under a demographically informed prior. The corrected effective entropy is
\begin{equation}
...
GeoVault assumes that a user selects a finite set of geographic points. \add{Selection is performed by clicking or tapping a location on a digital map interface; no GPS or device location service is involved. The geospatial encoding scheme (\S\ref{sec:spatial_models}) immediately snaps any selected coordinate to the nearest discrete cell, so sub-cell coordinate imprecision is absorbed by design. GPS-induced drift is therefore not a concern in the GeoVault threat model.} Formally, the set of selected locations is defined in equation \ref{eq:spatial_points}.
\begin{equation}
...
\add{GeoVault exposes three independent security axes, each with a distinct cost profile for the defender and the attacker.}

\add{\textbf{(1) Number of spatial points $n$.} The attacker's search space grows \emph{exponentially} in $n$ (as $|\mathcal{D}|^n$), while the legitimate user's key derivation cost remains \emph{constant}---a single Argon2id invocation regardless of how many points are selected. Each additional memorable location multiplies the attacker's workload by $|\mathcal{D}| \approx 2^{45.7}$ under a uniform prior, at zero additional computational cost to the user. This axis provides the most favorable asymmetry and is the primary means of reaching the \textbf{Super Secure} zone.}

\add{\textbf{(2) Memory parameter $m$.} Increasing the Argon2id memory cost raises the per-candidate evaluation cost for both parties, but the attacker's throughput collapses non-linearly once $m$ exceeds available GPU VRAM. This hardware-enforced bottleneck is analyzed empirically in Section~5 and constitutes the primary axis for users who can memorize only a single location.}

\add{\textbf{(3) KDF chaining depth $k$.} A user may chain $k$ Argon2id invocations in series, where each call's output becomes the \emph{password input} of the next (Equation~\ref{eq:kdf_chaining}). Because each invocation re-initializes the full $m$-byte memory block from scratch using the previous output as the new password, chaining $k$ calls is \emph{not} equivalent to multiplying the iteration count $t$ by $k$; it is strictly more expensive per unit due to repeated memory initialization. Crucially, the strict serial data dependency---each call requires the output of the previous one---means the attacker cannot exploit intra-candidate parallelism to shortcut any of the $k$ invocations. Both defender and attacker costs scale linearly with $k$, so this axis does not improve the defender-attacker \emph{ratio}. However, it raises the absolute computational floor by a factor of $k$ and is directly accessible to a user with $n=1$ who wishes to extend their security margin without memorizing additional locations or modifying system parameters---the user simply decides how many seconds of waiting they are willing to accept. These three axes are orthogonal and can be combined freely to meet a given threat model and cognitive budget.}
...
\paragraph{\add{Offline resolvability and open alternatives.}} \add{It should be noted that, while What3Words provides excellent usability, its word-to-coordinate resolution depends on a proprietary algorithm and, in standard deployments, requires access to the What3Words API or a bundled proprietary SDK. This creates a tension with the offline resolvability requirement stated above. For deployments requiring strict offline operation, \emph{Open Location Code} (OLC/Plus\,Codes)~\cite{Mai2022} constitutes the recommended alternative: it is fully open, requires no external service, and satisfies all three encoder properties natively. At precision level~11 (cell footprint $\approx 9\,\mathrm{m}^2$), OLC achieves a nominal single-point entropy comparable to the What3Words baseline values used throughout this paper. The What3Words instantiation is retained here to represent the usability design space; the security model and all quantitative results are independent of the specific encoder choice.}
...
\paragraph{Canonical ordering and usability.} \add{The concatenation in Equation~\ref{eq:spatial_concat} is order-sensitive: swapping two identifiers produces a different string $S$ and therefore an entirely different key $K$. This is a deliberate design choice that preserves the full $n!$ permutation space as part of the secret, but it imposes a usability requirement: the user must recall both the locations \emph{and} the sequence in which they were originally entered. Entering the same locations in a different order will silently produce an incorrect key. The ordered construction is treated as the reference throughout this paper; future implementations could adopt lexicographic sorting to produce an order-invariant input, at the cost of $\log_2 n!$ bits of permutation entropy (e.g., $\approx 1.6$ bits for $n=3$).}
...
\paragraph{\add{Fixed salt and rainbow table resistance.}} \add{The use of a fixed, public salt is an intentional design decision consistent with established practice. Notably, the BIP-39 standard employs the same approach: wallet keys are derived via $\mathsf{PBKDF2}(\textit{mnemonic},\,\texttt{"mnemonic"},\,2048)$, where the salt is the fixed public string \texttt{"mnemonic"} (optionally appended with a user passphrase). Rainbow table attacks are infeasible against GeoVault for two independent reasons. First, constructing a rainbow table requires precomputing $\mathsf{Argon2id}$ over the full input space; at the reference memory parameter ($m = 4\,\mathrm{GiB}$), each evaluation occupies $4\,\mathrm{GiB}$ of high-bandwidth memory, making a table covering even $2^{40}$ entries physically unrealizable with current hardware. Second, the nominal spatial entropy of the KDF input reaches $45.7n$ bits for $n$ selected points (Section~3.2), yielding $2^{45.7}$ candidates per additional point---a space orders of magnitude beyond any feasible precomputation budget. The fixed salt therefore introduces no exploitable weakness beyond what the spatial entropy model and the KDF cost parameters already account for.}

\paragraph{\add{Protocol-fixed parameters and user cognitive load.}} \add{To eliminate any additional memory burden on the user, the GeoVault protocol designates $(t, m, p)$ as \emph{protocol-level constants}, analogous to the fixed 2048-iteration count in BIP-39. These values are defined in the protocol specification and are never required to be recalled by the user. The only user-facing security parameter is the chaining depth $k$ (Equation~\ref{eq:kdf_chaining}), which defaults to $k=1$ and may be increased by the user as a single memorable integer. Consequently, the complete derivation state a user must retain is: (1)~their spatial point(s), and (2)~optionally, $k$ if they have chosen to increase it. The empirical evaluation in Section~5 surveys the full spectrum of $(t, m, p)$ configurations to characterize attacker throughput across the parameter space; the reference protocol parameters are selected from this spectrum to provide a target defender latency of approximately one second on commodity hardware.}
...
In practice, the attacker’s guess rate $R$ is instantiated using measured GPU throughput, while $C$ is determined by the cost of the key derivation function (KDF) or hash evaluation used by the scheme. For constructions employing a memory-hard KDF such as Argon2 with parameters $(t, m, p)$ and chaining depth $k$, the attacker must perform $k$ sequential Argon2id invocations per candidate (Section~4.1, Equation~\ref{eq:kdf_chaining}), yielding the attacker-adjusted work factor in equation \ref{eq:empirical_workfactor}.
\begin{equation}
\mathcal{W}_{\text{attacker}}(H, t, m, p, k) \approx \frac{k \cdot 2^{H}}{R_{\text{GPU,Argon2}}(t, m, p)}.
\label{eq:empirical_workfactor}
\end{equation}
\add{Since the serial data dependency in Equation~\ref{eq:kdf_chaining} prevents the attacker from parallelising across the $k$ rounds for a single candidate, the effective GPU throughput for a chained evaluation is $R_{\text{GPU,Argon2}}(t,m,p)/k$, and the work factor scales linearly with $k$. All results reported in Section~5 correspond to $k=1$; the work factor for any $k > 1$ is obtained by multiplying the reported values by $k$.}
...
\add{Attacker-side benchmarks used configurations maximising effective guess throughput under the memory-bound parallelism constraint of Section~\ref{sec:security_evaluation}.}
...
\add{Targeted (home city known)} & \add{$\le 1.57\times10^{3}$} & \add{$\le 27.38$} & \add{$\ge 18.31$} \\
\bottomrule
\end{tabular}
\end{table}

Crucially, even under the pessimistic Omnibus prior, the effective entropy remains $\approx 41.4$ bits, substantially higher than typical human-chosen passwords, which empirical studies frequently place below $\approx 20$ effective bits under adaptive guessing models~\cite{Bonneau2012}. \add{The 41.38-bit Omnibus value refers to \emph{spatial} entropy alone and does not determine security zone membership. Zone classification in GeoVault is based on the attacker-adjusted work factor $\mathcal{W}$, which incorporates the Argon2id cost. As shown in Table~\ref{tab:combined_spatial_computational}, an $n=1$ secret with Omnibus spatial entropy reaches the Human-Scale Secure zone at $m \geq 1{,}024$\,MiB; at $m=4{,}096$\,MiB, $\mathcal{W} > 10^{16}$\,s --- more than six orders of magnitude above the Insecure threshold.}

\add{Across all spatial dictionaries evaluated in Table~\ref{tab:entropy_dictionaries}, the effective per-point entropy spans the range $[27.38,\;41.38]$\,bits, bounded below by the Urban (city-scale) prior and above by the Omnibus HPZ prior. For an $n$-point secret under independent-point selection, the total effective entropy therefore satisfies
\begin{equation}
H_{\text{eff}}(n) \in [27.4\,n,\;41.38\,n]\;\text{bits},
\label{eq:entropy_bracket}
\end{equation}
where the lower bound corresponds to a demographically informed attacker with city-level knowledge of the user's location, and the upper bound corresponds to a geographically uninformed attacker restricted to the broadest realistic human-preferred zone. No attacker prior evaluated in this work contracts the per-point entropy below 27.4\,bits without sub-neighbourhood-scale knowledge of the user's location, confirming that this bracket spans the full range of evaluated threat models.}
...
\begin{tabular}{lrrrr}
\toprule
\textbf{Memory setting} & \textbf{Mean (ms)} & \textbf{$\sigma$ (ms)} & \textbf{P50 (ms)} & \textbf{P95 (ms)} \\
\midrule
64 MiB   & 74.0        & 5.2         & 72.4        & 87.1        \\
256 MiB  & 303.2       & 3.3         & 303.4       & 308.5       \\
1024 MiB & 1{,}234.9   & 47.1        & 1{,}219.8   & 1{,}391.5   \\
8192 MiB & 10{,}534.3  & 620.9       & 10{,}269.6  & 12{,}063.0  \\
\bottomrule
\multicolumn{5}{l}{\footnotesize CV\,$=$\,3.0\%--7.0\% (memory-bandwidth jitter under OS scheduling; see text).}
\end{tabular}
\end{table}

\paragraph{Consumer hardware extrapolation.}
\add{The latencies in Table~\ref{tab:argon2_defender_cost} were measured on a server-class platform (Intel Xeon Gold 6338, quad-channel DDR4, $\approx 204$\,GB/s peak memory bandwidth). Since Argon2id throughput is dominated by memory-bandwidth rather than compute, defender latency scales approximately inversely with available bandwidth~\cite{7467361}. A typical consumer laptop CPU (e.g., Intel Core i7 12th-generation with dual-channel DDR5~\cite{JEDEC2020DDR5}) delivers approximately 50\% of the Xeon Gold 6338 peak bandwidth, yielding an estimated scaling factor of $\approx 2\times$. Mobile devices (e.g., ARM Cortex-X class SoC with LPDDR5x, $\approx 65$\,GB/s peak bandwidth~\cite{JEDEC2022LPDDR5x}) deliver approximately 32\% of the server platform bandwidth, yielding a scaling factor of $\approx 4\times$ and an estimated latency of $\approx 4.9$\,s at $m=1{,}024$\,MiB---within a tolerable range for infrequent cold-storage key derivation. Memory tiers above $m=1{,}024$\,MiB may exceed the available RAM on many smartphones (typically 8--12\,GiB) and should not be used on mobile devices without verifying available system memory. Table~\ref{tab:argon2_defender_cost_consumer} reports the projected latency for all evaluated memory tiers across defender hardware classes.}

\begin{table}[H]
\caption{\add{Projected Argon2id defender latency on consumer and mobile hardware ($t=1$, $p=1$, $k=1$). Laptop estimate uses a $\approx 2\times$ bandwidth scaling factor; smartphone estimate uses a $\approx 4\times$ scaling factor relative to the Intel Xeon Gold 6338 server platform. Entry marked --- exceeds typical smartphone RAM.}}
\label{tab:argon2_defender_cost_consumer}
\centering
\begin{tabular}{lrrr}
\toprule
\textbf{Memory $m$} & \textbf{T\textsubscript{server} (s)} & \textbf{T\textsubscript{laptop} (s)} & \textbf{T\textsubscript{phone} (s)} \\
\midrule
64\,MiB      & 0.07  & 0.15          & 0.28          \\
256\,MiB     & 0.30  & 0.63          & 1.20          \\
1{,}024\,MiB & 1.23  & \textbf{2.6}  & \textbf{4.9}  \\
8{,}192\,MiB & 10.5  & 22.1          & ---           \\
\bottomrule
\multicolumn{4}{l}{\footnotesize Reference configuration ($m=1{,}024$\,MiB, bold) is within tolerable range on both laptop and phone for cold-storage use.}
\end{tabular}
\end{table}

\add{Table~\ref{tab:argon2_defender_cost} reports single-invocation latency ($k=1$). When the user applies KDF chaining at depth $k$ (Equation~\ref{eq:kdf_chaining}), the total key-derivation time scales linearly as $T_\text{CPU}(k) = k \cdot T_\text{CPU}^{(1)}$. Table~\ref{tab:kchain_empirical} reports empirically measured chain latency for all powers-of-two depths at the $m=1{,}024$\,MiB reference configuration ($n=50$ reps each). The measured ratios $T(k)/(k\cdot T(1))$ lie within~2\% of unity across all $k\in\{1,2,4,8,16,32,64\}$, confirming linearity.}

\begin{table}[H]
\caption{\add{KDF chain latency by depth ($m=1{,}024$\,MiB, $t=1$, $p=1$, $n=50$ reps each). All rows empirically measured; ratio $= T(k)/(k\cdot T(1))$ confirms linearity to within~2\%.}}
\label{tab:kchain_empirical}
\centering
\begin{tabular}{rrrr}
\toprule
\textbf{Depth $k$} & \textbf{Mean (s)} & \textbf{$\sigma$ (s)} & \textbf{Ratio} \\
\midrule
 1 &  1.255 & 0.006 & 1.000 \\
 2 &  2.526 & 0.046 & 1.006 \\
 4 &  5.051 & 0.053 & 1.006 \\
...
For linguistic mnemonics, an offline attacker evaluates candidate phrases using PBKDF2--HMAC--SHA512 as specified by the BIP-39 standard. Hashcat benchmarks were scaled to the full 2{,}048-iteration configuration. The measured sustained throughput is reported in Table~\ref{tab:attacker_cost}:
\[
...
\bottomrule
\multicolumn{4}{l}{\footnotesize GPU throughput is highly deterministic: P95\,$=$\,Median for all tiers at or above 2{,}048\,MiB;}\\
\multicolumn{4}{l}{\footnotesize CV\,$<\,$0.1\% for all rows. No explicit $\sigma$ column is needed for attacker-side measurements.}
\end{tabular}
\end{table}
...
\paragraph{\add{Defender--Attacker Throughput Asymmetry.}}
\add{To characterize the net advantage an attacker holds per defender authentication event, we define the \emph{GPU throughput advantage}:
\begin{equation}
    \eta(m) = R_\text{GPU}(m) \cdot T_\text{CPU}(m)
    \label{eq:eta}
\end{equation}
where $R_\text{GPU}(m)$ is the attacker GPU guess rate (H/s) and $T_\text{CPU}(m)$ is the defender CPU key-derivation latency (s). The quantity $\eta(m)$ is the number of attacker guesses that can be tested during the time it takes the legitimate user to perform one key derivation. A value $\eta > 1$ means the attacker gains ground on every login attempt. Table~\ref{tab:eta} reports $\eta$ for the measured Argon2id tiers.

Two structural properties of $\eta$ are critical to GeoVault's security argument.
First, \emph{KDF chaining depth $k$ leaves $\eta$ unchanged.} Since $R_\text{GPU}^{(k)} = R_\text{GPU}^{(1)}/k$ and $T_\text{CPU}^{(k)} = k\cdot T_\text{CPU}^{(1)}$, the product $\eta(m,k) = R_\text{GPU}^{(1)}\cdot T_\text{CPU}^{(1)} = \eta(m,1)$. Chaining imposes a symmetric tax: it raises the absolute cost of each guess but leaves the ratio unchanged.
Second, \emph{spatial dimensionality $n$ is orthogonal to $\eta$.} Adding a spatial point multiplies the search space by $\approx\!2^{26.8}$ per point (Table~\ref{tab:raw_radius_entropy}) without affecting either $R_\text{GPU}$ or $T_\text{CPU}$. Even with $\eta = 66$ at $m=1{,}024$\,MiB, an $n=3$ configuration ($\mathcal{W}\approx 3.8\times10^{39}$\,s) requires $3.8\times10^{39}/66 \approx 5.8\times10^{37}$\,s under a coordinated 66-GPU attack---still solidly in the Super Secure zone. The spatial entropy margin absorbs the attacker throughput advantage by orders of magnitude.}

\begin{table}[H]
\caption{\add{GPU throughput advantage $\eta(m) = R_\text{GPU}(m)\cdot T_\text{CPU}(m)$: attacker guesses per one defender authentication event. KDF chaining depth $k$ does not change $\eta$; only $m$ drives it downward. Defender latencies from Table~\ref{tab:argon2_defender_cost}; attacker rates from Table~\ref{tab:argon2_spectrum}.}}
\label{tab:eta}
\centering
\begin{tabular}{rrrr}
\toprule
\textbf{Memory $m$} & \textbf{$R_\text{GPU}$ (H/s)} & \textbf{$T_\text{CPU}$ (s)} & \textbf{$\eta$} \\
\midrule
64\,MiB      & 4985  & 0.074 & 369 \\
256\,MiB     & 802   & 0.303 & 243 \\
1{,}024\,MiB & 53.7  & 1.235 &  66 \\
8{,}192\,MiB & 0.54  & 10.53 & 5.7 \\
32{,}768\,MiB & 0.03 & $\approx$82 & $\approx$2.5 \\
\bottomrule
\multicolumn{4}{l}{\footnotesize $k$-chaining leaves $\eta$ unchanged; spatial points $n$ add entropy without affecting $\eta$.}
\end{tabular}
\end{table}

\paragraph{\add{Argon2id Throughput Under KDF Chaining.}}
\add{Table~\ref{tab:argon2_kchain_attacker} reports derived attacker throughput under KDF chaining at the $m=1{,}024$\,MiB reference tier, computed as $R_\text{GPU}^{(k)} = R_\text{GPU}^{(1)} / k$ with $R_\text{GPU}^{(1)} = 53.7$\,H/s (Section~\ref{sec:defender_performance}; CPU linearity validates the formula, Table~\ref{tab:kchain_empirical}; all time-to-compromise values are conservative lower bounds).}

\begin{table}[H]
\caption{\add{Derived attacker Argon2id throughput under KDF chaining ($m=1{,}024$\,MiB, $t=1$, $p=1$). Values computed as $R^{(k)} = R^{(1)}/k$ with $R^{(1)}=53.7$\,H/s; conservative upper bound on true attacker rate (CPU linearity ratios $\leq 1.029$, Table~\ref{tab:kchain_empirical}).}}
\label{tab:argon2_kchain_attacker}
\centering
\begin{tabular}{rrl}
\toprule
\textbf{Depth $k$} & \textbf{$R^{(k)}$ (H/s)} & \textbf{Basis} \\
\midrule
 1 & 53.70 & measured \\
...
64 &  0.84 & derived \\
\bottomrule
\multicolumn{3}{l}{\footnotesize CPU ratios confirm true $R^{(k)} \leq R^{(1)}/k$; time-to-compromise values are lower bounds.}
\end{tabular}
\end{table}


\paragraph{\add{ASIC Attack Resistance.}}
\add{A natural concern is whether purpose-built application-specific integrated circuits (ASICs) could exceed the measured GPU attacker throughput and invalidate the work-factor estimates. For Argon2id, this is fundamentally limited by the memory-bandwidth bottleneck rather than compute throughput. The Argon2 specification formally establishes that any implementation allocating less than the nominal memory $m$ incurs an exponential time penalty through mandatory data-dependent recomputation, precluding any space--time trade-off~\cite{7467361}. A hypothetical Argon2id ASIC must therefore provision high-bandwidth memory comparable to the target GPU; savings come only from eliminating general-purpose execution overhead in the compute fabric, which the Argon2 designers estimate yields a practical speedup of approximately $2$--$4\times$ over GPU-class hardware for realistic ASIC implementations~\cite{7467361,Wetzels2016}. Applying a conservative $10\times$ ASIC advantage to the reference attacker rate ($R_{\text{GPU}} = 53.7$\,H/s at $m=1{,}024$\,MiB) yields $R_{\text{ASIC}} \leq 537$\,H/s. This reduces all reported Argon2id attacker-adjusted work factors by at most $\log_2(10) \approx 3.3$\,bits. At $n=3$ spatial points, the baseline work factor exceeds $10^{39}$\,s (Super Secure zone, Table~\ref{tab:combined_spatial_computational}); a 3.3-bit reduction leaves it at $>10^{38}$\,s, well within the same zone. For the $n=1$ case at $m=1{,}024$\,MiB the work factor remains in the Human-Scale Secure zone even under this pessimistic ASIC bound. The GPU benchmark therefore provides a conservative upper bound on adversarial throughput under memory-hard protection, and ASIC-specific threats do not materially alter the security conclusions of Sections~\ref{sec:hardened_analysis}--\ref{sec:defense_in_depth}.}
...
\begin{table}[H]
\caption{Time-to-compromise under Argon2id hardening for linguistic ($\approx$20 bits) and single-point spatial secrets ($n=1$, $\approx$45.7 bits).}
...
% =================================================================
\subsection{\add{Effect of KDF Chaining Depth on Brute-Force Resistance}}
\label{sec:kchain_analysis}
\label{sec:kchain_spectrum}

\add{KDF chaining (Equation~\ref{eq:kdf_chaining}) provides a user-controlled work factor multiplier that scales both sides of the security equation linearly. Because each invocation reuses the full $m$-byte memory footprint and the serial data dependency prevents per-candidate parallelism, attacker throughput drops to $R_\text{GPU}^{(k)} = R_\text{GPU}^{(1)} / k$ (Table~\ref{tab:argon2_kchain_attacker}; conservative upper bound---CPU linearity ratios $\leq 1.029$ confirm the true attacker rate satisfies $R^{(k)} \leq R^{(1)}/k$) while defender latency grows to $T_\text{CPU}(k) = k \cdot T_\text{CPU}^{(1)}$ (Table~\ref{tab:kchain_empirical}). Table~\ref{tab:k_chaining_effect} quantifies this trade-off at two memory tiers ($m=1{,}024$\,MiB and $m=4{,}096$\,MiB); all attacker time-to-compromise values are lower bounds on actual security.}

\begin{table}[H]
\caption{\add{Effect of KDF chaining depth $k$ on attacker time-to-compromise and defender wait time, at $m=1{,}024$\,MiB ($R^{(1)}\approx53.7$\,H/s, $T_\text{CPU}^{(1)}\approx1.3$\,s) and $m=4{,}096$\,MiB ($R^{(1)}\approx2.70$\,H/s, $T_\text{CPU}^{(1)}\approx10$\,s). Defender wait = $k\times T_\text{CPU}^{(1)}$; attacker columns derived from $R(k)=R(1)/k$. Cell shading indicates security zone.}}
\label{tab:k_chaining_effect}
\centering
\begin{tabular}{rrrrrr}
\toprule
\textbf{Chaining} & \multicolumn{2}{c}{\textbf{Defender wait}} & \textbf{Password} & \multicolumn{2}{c}{\textbf{Spatial, $n=1$ ($\approx$45.7 bits)}} \\
\cmidrule(lr){2-3}\cmidrule(lr){5-6}
\textbf{depth $k$} & \textbf{1\,GiB} & \textbf{4\,GiB} & \textbf{($\approx$20 bits)} & \textbf{1\,GiB} & \textbf{4\,GiB} \\
\midrule
...
\bottomrule
\multicolumn{6}{l}{\footnotesize Password column uses 1\,GiB attacker rate; 4\,GiB values are $20\times$ larger but remain in the Insecure zone.}
\end{tabular}
\end{table}

\add{Three conclusions follow. First, for low-entropy linguistic secrets ($\approx$20 bits), chaining provides no practical rescue at either memory tier: even at $k=64$ the expected time-to-compromise remains below $10^{10}$\,s (Insecure zone), because the entropy deficit is too large for a linear multiplier to overcome. Second, for single-point spatial secrets, increasing $m$ from 1\,GiB to 4\,GiB multiplies the work factor by $\approx\!20\times$ at every $k$ level---a direct consequence of the $20\times$ reduction in attacker throughput---yet all entries remain in the Human-Scale Secure zone. \emph{No combination of $k$ and $m$ evaluated here lifts an $n=1$ configuration to the Super Secure zone}; that threshold requires additional spatial points (Section~\ref{sec:defense_in_depth}). Third, the 4\,GiB column makes the defender cost explicit: the 10\,s per-invocation wait is acceptable for cold-storage or archive scenarios, but is above the interactive 2\,s budget at $k>1$. Users with a strict latency constraint should prefer $m=1{,}024$\,MiB with higher $k$, or add a second spatial point, over increasing $m$ to 4\,GiB.}
...
\remove{We analyze the combined effect of spatial multi-point selection and memory-hard key derivation on brute-force resistance.}\add{We analyze the joint effect of KDF chaining depth $k$, multi-point spatial selection, and memory-hard key derivation on brute-force resistance. Since each factor independently multiplies the attacker work factor, their combination yields compounding security gains without additional memorization burden.} As in the preceding subsections, spatial entropy is treated as a fixed input parameter (Section~\ref{sec:clustering}), and security is evaluated exclusively via the attacker-adjusted work factor $\mathcal{W}$ defined in Equation~\ref{eq:work_factor_attacker}.

Figure~\ref{fig:hardening_spectrum} visualizes the combined effect: the Y-axis encodes the number of spatial points $n$ (top panel) or password length in characters (bottom panel), the X-axis is the attacker work factor on a log scale, and each colored line corresponds to a different Argon2id memory tier. The figure instantiates Equation~\ref{eq:empirical_workfactor} using the empirically measured throughput values of Section~\ref{sec:hardware_benchmarks}.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{img/Hardening_Spectrum_No16G.pdf}
    \caption{Attacker-adjusted brute-force resistance under Argon2id hardening. Top: spatial points $n$ (Y-axis) vs.\ attacker work factor; bottom: password length (Y-axis) vs.\ attacker work factor. Each line is a memory tier $m$; security zones shaded.}
