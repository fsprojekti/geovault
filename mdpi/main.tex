...
\subsection{ Encryption Techniques for Hardening Secrets}
...
Equation~\ref{eq:spatial_dict_entropy} models the attacker's \emph{geographic} prior---which dictionary stratum is targeted---but treats selection \emph{within} the targeted stratum as uniform. Empirical studies of geographic authentication systems demonstrate that this uniformity assumption overstates effective entropy: users disproportionately choose home addresses, workplaces, frequently visited venues, and culturally salient landmarks, concentrating realistic choices over a sub-fraction of the dictionary's nominal coverage area~\cite{Sun2012,Thorpe2013,AlAmeen2014}. To incorporate this within-dictionary behavioural bias, we introduce a selection fraction $f_s \in (0,1]$, representing the proportion of cells in $\mathcal{D}$ that attract non-negligible probability mass under a demographically informed prior. The corrected effective entropy is
\begin{equation}
...
GeoVault assumes that a user selects a finite set of geographic points. Selection is performed by clicking or tapping a location on a digital map interface; no GPS or device location service is involved. The geospatial encoding scheme (\S\ref{sec:spatial_models}) immediately snaps any selected coordinate to the nearest discrete cell, so sub-cell coordinate imprecision is absorbed by design. GPS-induced drift is therefore not a concern in the GeoVault threat model. Formally, the set of selected locations is defined in equation \ref{eq:spatial_points}.
\begin{equation}
...
\end{equation}
Since the serial data dependency in Equation~\ref{eq:kdf_chaining} prevents the attacker from parallelising across the $k$ rounds for a single candidate, the effective GPU throughput for a chained evaluation is $R_{\text{GPU,Argon2}}(t,m,p)/k$, and the work factor scales linearly with $k$. All results reported in Section~5 correspond to $k=1$; the work factor for any $k > 1$ is obtained by multiplying the reported values by $k$.
...
Attacker-side benchmarks used configurations maximising effective guess throughput under the memory-bound parallelism constraint of Section~\ref{sec:security_evaluation}.
...
The latencies in Table~\ref{tab:argon2_defender_cost} were measured on a server-class platform (Intel Xeon Gold 6338, quad-channel DDR4, $\approx 204$\,GB/s peak memory bandwidth). Since Argon2id throughput is dominated by memory-bandwidth rather than compute, defender latency scales approximately inversely with available bandwidth~\cite{7467361}. A typical consumer laptop CPU (e.g., Intel Core i7 12th-generation with dual-channel DDR5) delivers approximately 40--60\% of the Xeon Gold 6338 peak bandwidth, yielding an estimated scaling factor of $1.7\times$--$2.5\times$. Table~\ref{tab:argon2_defender_cost_consumer} reports the resulting projected latency range for the four evaluated memory tiers. At the reference protocol configuration ($m=1{,}024$\,MiB), the estimated defender latency on a consumer laptop is $2.1$--$3.1$\,s---elevated compared to the server benchmark but still within the sub-four-second interactive budget~\cite{nielsen1994usability}. Tiers up to $m=8{,}192$\,MiB are physically realisable on laptops with at least 16\,GiB of system RAM; larger tiers require dedicated workstation hardware and are appropriate only for cold-storage or archival scenarios.

\begin{table}[H]
\caption{Projected Argon2id defender latency on consumer hardware ($t=1$, $p=1$, $k=1$). Estimates derived from Table~\ref{tab:argon2_defender_cost} using a $1.7\times$--$2.5\times$ memory-bandwidth scaling factor for a consumer laptop versus the Intel Xeon Gold 6338 server platform.}
\label{tab:argon2_defender_cost_consumer}
\centering
\begin{tabular}{lrrrl}
\toprule
\textbf{Memory $m$} & \textbf{T\textsubscript{server} (s)} & \textbf{T\textsubscript{laptop,low} (s)} & \textbf{T\textsubscript{laptop,high} (s)} & \textbf{Laptop RAM needed} \\
\midrule
64\,MiB   & 0.07  & 0.12 & 0.19 & $\geq$4\,GiB \\
256\,MiB  & 0.30  & 0.51 & 0.76 & $\geq$4\,GiB \\
1{,}024\,MiB & 1.23 & \textbf{2.1} & \textbf{3.1} & $\geq$4\,GiB \\
8{,}192\,MiB & 10.5 & 17.6 & 26.3 & $\geq$16\,GiB \\
\bottomrule
\multicolumn{5}{l}{\footnotesize Reference configuration ($m=1{,}024$\,MiB, bold) is comfortably interactive on consumer hardware.}
\end{tabular}
\end{table}
...
\begin{table}[H]
\caption{KDF chain latency by depth ($m=1{,}024$\,MiB, $t=1$, $p=1$, $n=50$ reps each). All rows empirically measured; ratio $= T(k)/(k\cdot T(1))$ confirms linearity to within~2\%.}
...
To characterize the net advantage an attacker holds per defender authentication event, we define the \emph{GPU throughput advantage}:
\begin{equation}
...
\begin{table}[H]
\caption{Derived attacker Argon2id throughput under KDF chaining ($m=1{,}024$\,MiB, $t=1$, $p=1$). Values computed as $R^{(k)} = R^{(1)}/k$ with $R^{(1)}=53.7$\,H/s; conservative upper bound on true attacker rate (CPU linearity ratios $\leq 1.029$, Table~\ref{tab:kchain_empirical}).}
...
KDF chaining (Equation~\ref{eq:kdf_chaining}) provides a user-controlled work factor multiplier that scales both sides of the security equation linearly. Because each invocation reuses the full $m$-byte memory footprint and the serial data dependency prevents per-candidate parallelism, attacker throughput drops to $R_\text{GPU}^{(k)} = R_\text{GPU}^{(1)} / k$ (Table~\ref{tab:argon2_kchain_attacker}; conservative upper bound---CPU linearity ratios $\leq 1.029$ confirm the true attacker rate satisfies $R^{(k)} \leq R^{(1)}/k$) while defender latency grows to $T_\text{CPU}(k) = k \cdot T_\text{CPU}^{(1)}$ (Table~\ref{tab:kchain_empirical}). Table~\ref{tab:k_chaining_effect} quantifies this trade-off at the $m=1{,}024$\,MiB reference configuration; all attacker time-to-compromise values are therefore lower bounds on actual security.

\begin{table}[H]
\caption{Effect of KDF chaining depth $k$ on attacker time-to-compromise and defender wait time at $m=1{,}024$\,MiB ($t=1$, $p=1$, $R^{(1)}\approx53.7$\,H/s). Defender wait times from empirical measurements ($n=50$); attacker columns derived from $R(k)=R(1)/k$. Cell shading indicates security zone.}
\label{tab:k_chaining_effect}
\centering
\begin{tabular}{rrrr}
\toprule
\textbf{Chaining} & \textbf{Defender} & \textbf{Password} & \textbf{Spatial, $n=1$} \\
\textbf{depth $k$} & \textbf{wait} & \textbf{($\approx$20 bits)} & \textbf{($\approx$45.7 bits)} \\
\midrule
 1 &  1.3\,s & \cellcolor{insecure}5.4\,h    & \cellcolor{humanscale}$3.3\times10^{4}$\,yr \\
 2 &  2.5\,s & \cellcolor{insecure}10.8\,h   & \cellcolor{humanscale}$6.6\times10^{4}$\,yr \\
 4 &  5.1\,s & \cellcolor{insecure}21.7\,h   & \cellcolor{humanscale}$1.3\times10^{5}$\,yr \\
 8 & 10.1\,s & \cellcolor{insecure}1.8\,days  & \cellcolor{humanscale}$2.6\times10^{5}$\,yr \\
16 & 20.3\,s & \cellcolor{insecure}3.6\,days  & \cellcolor{humanscale}$5.3\times10^{5}$\,yr \\
32 & 41.2\,s & \cellcolor{insecure}7.2\,days  & \cellcolor{humanscale}$1.1\times10^{6}$\,yr \\
64 & 82.5\,s ($\approx$1.4\,min) & \cellcolor{insecure}14.5\,days & \cellcolor{humanscale}$2.1\times10^{6}$\,yr \\
\bottomrule
\end{tabular}
\end{table}

Two conclusions follow directly. First, for low-entropy linguistic secrets ($\approx$20 bits), chaining provides no practical rescue: even at $k=64$ the expected time-to-compromise remains below $10^{10}$\,s (Insecure zone), because the entropy deficit is too large for a linear multiplier to overcome. Second, for spatial secrets the picture is reversed: the baseline $k=1$ value of $\approx\!33{,}400$ years is already firmly in the Human-Scale Secure zone, and chaining amplifies this proportionally at negligible memorization overhead. In the combined multi-point setting (Section~\ref{sec:defense_in_depth}), $k$ acts as a fine-grained dial that shifts the entire work-factor curve upward by $k$-fold without requiring additional spatial points or increased memory. Using powers of two for $k$ is consistent with standard cryptographic cost-parameter conventions, where each increment doubles the work factor.
...
GeoVault rests on a concrete premise: instead of forcing users to memorise abstract symbol sequences, derive keys from places they already know. The results confirm that this shift is quantitatively meaningful. Spatially anchored secrets carry a substantially higher effective entropy floor than human-chosen passwords under the same attacker priors, and Argon2id hardening amplifies this margin non-linearly as memory costs approach GPU VRAM capacity.
